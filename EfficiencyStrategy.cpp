...
bool compareFreightCapacity(const std::shared_ptr<Freight>& a,
    const std::shared_ptr<Freight>& b) {
    return a->getCapacity() > b->getCapacity();
}

// Comparator for cargo group size (descending order)
bool compareCargoGroup(const std::shared_ptr<Cargo>& a,
    const std::shared_ptr<Cargo>& b) {
...
void EfficiencyStrategy::schedule(FreightManager& fm,
    ...
    std::sort(freights.begin(), freights.end(), compareFreightCapacity);

    // Sort cargos by group size (largest groups first)
    std::sort(cargos.begin(), cargos.end(), compareCargoGroup);

    // Create a vector to track assigned cargos
    std::vector<bool> assigned(cargos.size(), false);
